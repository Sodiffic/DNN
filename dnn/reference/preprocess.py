"""
1. normalization
比较大小，然后在小的那一边进行随机的填充：eg【256，328】-》【328，328】所有x坐标都增加，然后都除以相同的数
2. 人数修改
    out_vec = [17个关节二维，2个人，300帧，17个质心二维][17,7,300,2]
    设置input_num=5, output_num=2：
    if member=1 随机在[0][1]添加人
    if member>5 list里加入前5个
        每一帧置信度相加求平均，找到前两个
    if frame>300 随机取完整一段
    if frame<300 从一开始循环
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--data_path', default='../data/kinetics_raw')  # 输入数据地址：.pkl
    parser.add_argument(
        '--out_folder', default='../data/kinetics')  # 输出地址
    ...
    for p in part:
        ...
        if not os.path.exists(arg.out_folder):
            os.makedirs(arg.out_folder)  # 不存在的话创建
